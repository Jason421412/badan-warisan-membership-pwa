...
export function EventDetails({ onNavigate, event }: EventDetailsProps) {
  ...
  const getMapEmbedUrl = () => {
    ...
    if (event.lat && event.lng) {
      return `https://www.google.com/maps/embed/v1/place?key=${googleMapsApiKey}&q=${event.lat},${event.lng}&zoom=15`;
    }
    return `https://www.google.com/maps/embed/v1/place?key=${googleMapsApiKey}&q=${encodeURIComponent(event.location)}`;
  };
        ...
        {(event.lat && event.lng) && (
          <div className="mb-6">
            ...
            {googleMapsApiKey ? (
              ...
              <div className="rounded-xl overflow-hidden h-48 bg-[#EAE8DE] flex items-center justify-center border border-[#B48F5E] border-opacity-30">
                <div className="text-center px-4">
                  <MapPin size={32} className="text-[#B48F5E] mx-auto mb-2 opacity-50" />
                  <p className="text-[#333333] font-['Inter'] text-sm">Interactive map area</p>
                  <p className="text-[#333333] opacity-60 font-['Inter'] text-xs mt-1">Map is disabled due to missing configuration.</p>
                </div>
              </div>
            )}
            <a 
